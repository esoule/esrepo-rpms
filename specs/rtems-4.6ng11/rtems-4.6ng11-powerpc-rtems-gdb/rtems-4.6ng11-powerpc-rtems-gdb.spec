...
Name:		rtems-4.6ng11-powerpc-rtems-gdb
...
Requires:	rtems-4.6ng11-gdb-common
...
%endif
  ...
  ../gdb-%{gdb_version}/configure \
    ...
    --with-gdb-datadir=%{_datadir}/powerpc-rtems-gdb-%{gdb_version} \
...
# Bug in gdb-7.0, bogusly installs linux-only files
  somethinguseful=0
  for f in ${RPM_BUILD_ROOT}%{_datadir}/powerpc-rtems-gdb-%{gdb_version}/syscalls/*.xml; do
    case $f in
    *linux.xml) rm -f $f;;
  ...
  if test $somethinguseful -eq 0; then
    rm -rf "${RPM_BUILD_ROOT}%{_datadir}/powerpc-rtems-gdb-%{gdb_version}/syscalls"
  fi

%if "{gdb_version}" >= "7.3"
%if ! %{with python}
# gdb-7.3 doesn't honor --without-python correctly
  rm -rf ${RPM_BUILD_ROOT}%{_datadir}/powerpc-rtems-gdb-%{gdb_version}/python
%endif
%endif
...
# powerpc-rtems-gdb
# ==============================================================
# %package -n rtems-4.6ng11-powerpc-rtems-gdb
...
%description -n rtems-4.6ng11-powerpc-rtems-gdb
GNU gdb targetting powerpc-rtems.

%files -n rtems-4.6ng11-powerpc-rtems-gdb
%{?el5:%defattr(-,root,root)}
%dir %{_prefix}
%dir %{_prefix}/share
%{?with_python:%{_datadir}/powerpc-rtems-gdb-%{gdb_version}}
...
# ==============================================================
# rtems-4.6ng11-gdb-common
# ==============================================================
%package -n rtems-4.6ng11-gdb-common
...
%description -n rtems-4.6ng11-gdb-common

GDB files shared by all targets.

%post -n rtems-4.6ng11-gdb-common
...
%preun -n rtems-4.6ng11-gdb-common
if [ $1 -eq 0 ]; then
...
%files -n rtems-4.6ng11-gdb-common
%{?el5:%defattr(-,root,root)}
%dir %{_prefix}
%dir %{_prefix}/share
...
%changelog
* Thu Jul 26 2014 Evgueni Souleimanov <esoule@100500.ca> - 7.7.1-1.1.1
- Build gdb 7.7.1 for developing with rtems-4.6 (rtems-4.6ng11)
